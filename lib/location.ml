...
let run_at t ~f ~prog ~args =
  ...
  | Remote { ssh_url; path = _ } ->
    ...
    f ~prog:"ssh" ~args:([ ssh_url; prog ] @ args)
;;

let run_lines_at = run_at ~f:(fun ~prog ~args -> Process.run_lines () ~prog ~args)
  ...
  run_at t ~f:(fun ~prog ~args ->
      ...
      let%bind () =
        ...
        |> Pipe.iter_without_pushback
             ~f:(Log.Global.info "%s" ~tags:[ "cmd", label; "fd", "stdout" ])
      and () =
        Process.stderr proc
        |> Reader.lines
        |> Pipe.iter_without_pushback
             ~f:(Log.Global.info "%s" ~tags:[ "cmd", label; "fd", "stderr" ])
      in
...
let run_with_output_to_writer_at t ~writer ~label =
  run_at t ~f:(fun ~prog ~args ->
      ...
      let open Deferred.Let_syntax in
      let%bind () =
        Pipe.transfer_id (Reader.pipe (Process.stdout proc)) (Writer.pipe writer)
      and () =
        Process.stderr proc
        |> Reader.lines
        |> Pipe.iter_without_pushback
...
let run_with_input_from_reader_at t ~reader ~label =
  run_at t ~f:(fun ~prog ~args ->
      ...
      let%bind () =
        let%bind () =
          Pipe.transfer_id (Reader.pipe reader) (Writer.pipe (Process.stdin proc))
        in
...
let list_snapshots t =
  let%map lines_or_error =
    run_lines_at t ~prog:"btrfs" ~args:[ "subvolume"; "list"; "-u"; "-R"; "-o"; path t ]
  in
  let open Or_error.Let_syntax in
  let%bind lines = lines_or_error in
  lines |> List.map ~f:parse_snapshot |> Or_error.all
;;
...
let send t ~snapshot ~available ~to_:writer =
  ...
  in
  run_with_output_to_writer_at
    t
    ~writer
    ~prog:"btrfs"
    ~args:([ "send" ] @ available_args @ [ path t ^/ Snapshot.name snapshot ])
    ~label:"send"
;;

let receive t ~from:reader =
  run_with_input_from_reader_at
    t
    ~reader
    ~prog:"btrfs"
    ~args:[ "receive"; path t ]
