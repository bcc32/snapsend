...
let sync ~from ~to_ ~delete_extraneous =
  ...
  and snapshots_to = Location.list_snapshots to_ in
  ...
  let%bind (_ : Snapshot.Set.t) =
    ...
    |> Deferred.Or_error.List.fold ~init:common ~f:(fun common snapshot ->
           ...
           let%map () =
             Location.send from ~snapshot ~available:(Set.to_list common) ~to_:writer
           and () = Location.receive to_ ~from:reader in
